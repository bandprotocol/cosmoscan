module Styles = {
  ...
  let typeContainer = w => style(list{marginRight(#px(20)), width(w)})

  let resolveIcon = style(list{width(#px(20)), height(#px(20)), marginLeft(Spacing.sm)})

  let hashContainer = style(list{maxWidth(#px(220))})
  let feeContainer = style(list{display(#flex), justifyContent(#flexEnd)})
  let timeContainer = style(list{display(#flex), alignItems(#center), maxWidth(#px(150))})
  let textContainer = style(list{display(#flex)})
  let countContainer = style(list{maxWidth(#px(80))})
  let proposerBox = style(list{maxWidth(#px(270)), display(#flex), flexDirection(#column)})
  let idContainer = style(list{display(#flex), maxWidth(#px(200))})
  let dataSourcesContainer = style(list{display(#flex)})
  let dataSourceContainer = style(list{display(#flex), width(#px(170))})
  let oracleScriptContainer = style(list{display(#flex), width(#px(170))})
  let resolveStatusContainer = style(list{
    display(#flex),
    alignItems(#center),
...
let renderHash = hash =>
  <div className=Styles.hashContainer>
    <Text
      block=true code=true value={hash |> Hash.toHex} size=Text.Lg weight=Text.Bold ellipsis=true
    />
  </div>
let renderAddress = address => <AddressRender address />
...
let renderTime = time => <div className=Styles.timeContainer> <Timestamp time size=Text.Md /> </div>
...
let renderDataSource = (id, name) =>
  <div className=Styles.dataSourceContainer>
    ...
    <Text
      value=name block=true height=Text.Px(16) spacing=Text.Em(0.02) nowrap=true ellipsis=true
    />
  </div>
...
let renderOracleScript = (id, name) =>
  <div className=Styles.oracleScriptContainer>
    ...
    <HSpacing size=Spacing.xs />
    <Text
      value=name block=true height=Text.Px(16) spacing=Text.Em(0.02) ellipsis=true nowrap=true
    />
  </div>
    ...
    <div className=Styles.dataSourcesContainer>
      ...
      ->Belt_List.map(id =>
        <div className=Styles.idContainer key={id |> ID.DataSource.toString}>
          <TypeID.DataSource id position=TypeID.Text /> <HSpacing size=Spacing.sm />
        </div>
      )
      ->Array.of_list
      ->React.array}
