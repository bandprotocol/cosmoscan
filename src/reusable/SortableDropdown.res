module Styles = {
  ...
  let sortDrowdownContainer = style(list{position(#relative), zIndex(2), maxWidth(#px(60))})
  ...
  let sortDropdownItem = (isActive, theme: Theme.t) =>
    style(list{
      backgroundColor(isActive ? theme.dropdownHover : theme.secondaryBg),
      transition(~duration=200, "all"),
      cursor(#pointer),
      padding2(~v=#px(8), ~h=#px(10)),
    })
  let sortDropdownTextItem = style(list{paddingRight(#px(15)), cursor(#pointer)})
