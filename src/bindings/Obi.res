...
let dataTypeToString = x =>
  switch x {
  | Params => "Params"
  | Result => "Result"
  }

let dataTypeToSchemaField = x =>
  switch x {
  | Params => "input"
  | Result => "output"
  }
...
let generateDecodeLibSolidity = (schema, dataType) => {
  ...
  let template = (structs, functions) =>
    j`library $(dataTypeString)Decoder {
...
`
  %Opt({
    let fieldsPairs = extractFields(schema, name)
    %Opt({
      let fields = fieldsPairs |> Belt_Array.map(_, parse) |> optionsAll
      let indent = "\n        "
      Some(
        template(
...
`
  ...
  %Opt({
    let paramsCode = paramsCodeOpt
    %Opt({
      let resultCode = resultCodeOpt
...
let declareGo = ({name, varType}) => {
  ...
  let type_ = switch varType {
  | Single(String) => j`string`
  | Single(U64) => j`uint64`
  | Single(U32) => j`uint32`
  | Single(U8) => j`uint8`
  | Array(String) => j`[]string`
  | Array(U64) => j`[]uint64`
  | Array(U32) => j`[]uint32`
  | Array(U8) => j`[]uint8`
  }
...
let generateDecoderGo = (packageName, schema, dataType) =>
  switch dataType {
  | Params => Some(j`"Code is not available."`)
  | Result =>
    let name = dataType |> dataTypeToSchemaField
...
\\t}, nil
    ...
    %Opt({
      let fieldsPair = extractFields(schema, name)
      %Opt({
        let fields = fieldsPair |> Belt_Array.map(_, parse) |> optionsAll
        Some(
          template(
