...
let getElementInList = (l, idx) => Belt_List.get(l, idx) |> Belt_Option.getExn
...
let comparePair = ((_, num1), (_, num2)) => compare(num1, num2)
...
let pythonMatch = str => {
  ...
  %Opt({
    let resultNullable = rawResult->Belt.Array.get(1)
    %Opt({
      let result = resultNullable->Js.Nullable.toOption
...
let getVariables = str => {
  ...
  %Opt({
    let program = splitedStr->Belt.List.get(0)
...
let parseExecutableScript = (buff: JsBuffer.t) => buff |> JsBuffer.toUTF8 |> getVariables
