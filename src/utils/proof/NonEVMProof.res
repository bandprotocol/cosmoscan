...
let decodeMultiStoreProof = json => {
  open JsonUtils.Decode
  {
    accToGovStoresMerkleHash: json |> field("accToGovStoresMerkleHash", string) |> JsBuffer.fromHex,
    mainAndMintStoresMerkleHash: json
    |> field("mainAndMintStoresMerkleHash", string)
    |> JsBuffer.fromHex,
    ...
    slashingToUpgradeStoresMerkleHash: json
    |> field("slashingToUpgradeStoresMerkleHash", string)
    |> JsBuffer.fromHex,
  }
}
...
let decodeBlockHeaderMerkleParts = json => {
  open JsonUtils.Decode
  {
    ...
    nextValidatorHashAndConsensusHash: json
    |> field("nextValidatorHashAndConsensusHash", string)
    |> JsBuffer.fromHex,
...
let rec encode = x =>
  ...
  | Proof({
    ...
    %Opt({
      let encodeMultiStore = encode(MultiStoreProof(multiStoreProof))
      %Opt({
        let encodeBlockHeaderMerkleParts = encode(BlockHeaderMerkleParts(blockHeaderMerkleParts))
        %Opt({
          let encodeSignatures = encode(Signatures(signatures))
          %Opt({
            let encodeReq = encode(RequestPacket(requestPacket))
            %Opt({
              let encodeRes = encode(ResponsePacket(responsePacket))
              %Opt({
                let encodeIAVLMerklePaths = encode(IAVLMerklePaths(iavlMerklePaths))
                Obi.encode(
                  ...
                  [
                    ...
                    {
                      fieldName: "packet",
                      fieldValue: JsBuffer.concat([encodeReq, encodeRes]) |> JsBuffer.toHex(
                        ~with0x=true,
                      ),
                    },
