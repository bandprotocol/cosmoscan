...
let create = (ledgerApp, accountIndex) => {
  ...
  %Promise({
    ...
    %Promise({
      let pubKeyInfo = LedgerJS.publicKey(app, path)
      %Promise({
        let appInfo = LedgerJS.appInfo(app)
        %Promise({
          let version = LedgerJS.getVersion(app)

          let {LedgerJS.major: major, minor, patch, test_mode, device_locked} = version
          ...
          if pubKeyInfo.return_code != 36864 {
            ...
            Promise.ret({transport: transport, app: app, path: path, prefix: prefix})
          }
        })
      })
    })
