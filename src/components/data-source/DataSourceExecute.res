module Styles = {
  ...
  let listContainer = style(list{marginBottom(#px(25))})
  ...
  let resultContainer = (theme: Theme.t) =>
    style(list{
      ...
      selector("> div + div", list{borderTop(#px(1), #solid, theme.tableRowBorderColor)}),
    })
  let resultBox = style(list{padding(#px(20))})
  let labelWrapper = style(list{
    flexShrink(0.),
    flexGrow(0.),
    flexBasis(#px(220)),
    Media.mobile(list{flexBasis(#px(100))}),
  })
  let resultWrapper = style(list{
    flexShrink(0.),
    flexGrow(0.),
    flexBasis(#calc(#sub, #percent(100.), #px(220))),
    Media.mobile(list{flexBasis(#calc(#sub, #percent(100.), #px(100)))}),
  })
}
...
let make = (~executable: JsBuffer.t) => {
  let params =
    ExecutableParser.parseExecutableScript(executable)->Belt_Option.getWithDefault(list{})
