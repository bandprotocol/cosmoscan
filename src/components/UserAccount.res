module Styles = {
  ...
  let logo = style(list{width(#px(12))})
  ...
  let connect = style(list{padding2(~v=#px(10), ~h=#zero)})
  let disconnect = style(list{paddingTop(#px(16))})
}
    ...
    <div className={Css.merge(list{CssHelper.flexBox(~justify=#flexEnd, ()), Styles.container})}>
      <div ref={ReactDOMRe.Ref.domRef(clickOutside)}>
        <div
          ...
          <div className={Styles.oval(theme)}> <Icon name="fal fa-user" color=Colors.white /> </div>
          <HSpacing size=Spacing.sm />
