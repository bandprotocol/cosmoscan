module Styles = {
  ...
  let container = style(list{paddingTop(#px(24)), paddingBottom(#px(10)), minHeight(#px(100))})
  ...
  let request = style(list{marginTop(#px(26)), marginRight(#px(12))})
}
...
let make = (~blockSub: ApolloHooks.Subscription.variant<BlockSub.t>) => {
  ...
  | Data({requests}) if requests->Belt.Array.length != 0 =>
    let onChainRequests = requests->Belt.Array.keepMap(({id, isIBC}) => !isIBC ? Some(id) : None)
    ...
    let showRequests = switch tabIndex {
    | 0 => onChainRequests
    | 1 => ibcRequests
    | _ =>
