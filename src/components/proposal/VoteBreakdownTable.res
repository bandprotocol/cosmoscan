module Styles = {
  ...
  let header = (theme: Theme.t) =>
    style(list{
      ...
      selector("> div + div", list{marginLeft(#px(32))}),
      Media.mobile(list{overflow(#auto), padding2(~v=#px(1), ~h=#px(15))}),
    })
  ...
  let make = (~reserveIndex, ~voteSub: ApolloHooks.Subscription.variant<VoteSub.t>) =>
    ...
    | _ =>
      <MobileCard
        values={
          open InfoMobileCard
          list{("Voter", Loading(230)), ("TX Hash", Loading(100)), ("Timestamp", Loading(100))}
        }
