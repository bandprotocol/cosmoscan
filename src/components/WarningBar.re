...
let make = () => {
  ...
  <div className={Css.merge([CssHelper.flexBox(~justify=`center, ()), Styles.bar])}>
    <div className={Css.merge([CssHelper.flexBox(~wrap=`nowrap, ()), Styles.innerBar])}>
        ...
        <Text
          value="Bandchain is operating normally.  Only cosmoscan is facing an issue and we are fixing it at the moment. This does not effect on other explorers or 3rd party wallets."
          block=true
