module Styles = {
  ...
  let upperTextCotainer = style(list{marginBottom(#px(24))})

  let listContainer = style(list{width(#percent(100.)), marginBottom(#px(25))})
  ...
  let withWH = (w, h) =>
    style(list{width(w), height(h), display(#flex), justifyContent(#center), alignItems(#center)})
  ...
  let mobileBlockContainer = style(list{padding2(~v=#px(24), ~h=#zero)})
  let mobileBlock = style(list{
    borderRadius(#px(4)),
    minHeight(#px(164)),
    selector("> i", list{marginBottom(#px(16))}),
  })
}
...
module ConnectPanel = {
  module Styles = {
    ...
    let connectInnerContainer = style(list{width(#percent(100.)), maxWidth(#px(370))})
  }
  @react.component
  let make = (~connect) => {
    ...
    let requestCallback = React.useCallback0(requestPromise => {
      ignore(
        ...
        |> Js.Promise.then_(res =>
          ...
          | _ =>
            setResult(_ => Error(
              "Fail to sign message, please connect with mnemonic or ledger first",
            ))
            Js.Promise.resolve()
          }
        )
                        ...
                        | Some(encoded) =>
                          ...
                          dispatch(
                            AccountContext.SendRequest({
                              ...
                              askCount: askCount,
                              minCount: minCount,
                              clientID: switch clientID |> String.trim == "" {
...
let make = (~id: ID.OracleScript.t, ~schema: string) =>
  %Opt({
    let paramsInput = schema->Obi.extractFields("input")
    Some(<ExecutionPart id schema paramsInput />)
  }) |> Belt.Option.getWithDefault(
    _,
    <MobileBlock>
      <Icon name="fal fa-exclamation-circle" size=32 /> <Text value="Schema not found" />
    </MobileBlock>,
  )
