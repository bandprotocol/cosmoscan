module Styles = {
  open Css
  let labelWrapper = style(list{flexShrink(0.), flexGrow(0.), flexBasis(#px(220))})
  let resultBox = style(list{padding(#px(20))})
  let withWH = (w, h) =>
    style(list{width(w), height(h), display(#flex), justifyContent(#center), alignItems(#center)})

  let proofContainer = style(list{
    selector(
      "> button + button",
      list{marginLeft(#px(24)), Media.mobile(list{marginLeft(#px(16))})},
    ),
  })
}
...
let make = (~id: ID.Request.t) => {
  ...
  <div className={Css.merge(list{CssHelper.flexBox(), Styles.resultBox})}>
    <div className=Styles.labelWrapper>
      <Text value="Proof of validaty" color=Colors.gray6 weight=Text.Regular height=Text.Px(15) />
    </div>
    {switch proofOpt {
    | Some(proof) =>
    ...
    | _ => <div className={Styles.withWH(#percent(100.), #auto)}> <Loading width=#px(104) /> </div>
    }}
  </div>
}
