...
let make = (~txResponse: TxCreator.tx_response_t, ~schema: string) =>
  {
    ...
    %Sub({
      let requestsByTxHash = requestsByTxHashSub
        ...
        <div className={Styles.resultContainer(theme)}>
          ...
          | Some({result: Some(result), id}) =>
            ...
            <>
              <div className={Css.merge(list{CssHelper.flexBox(), Styles.resultBox})}>
                <div className=Styles.labelWrapper>
                  <Text value="Output" color=Colors.gray6 weight=Text.Regular height=Text.Px(20) />
                </div>
          ...
          | Some(request) =>
            <div className={Css.merge(list{CssHelper.flexBox(), Styles.resultBox})}>
              <div className=Styles.labelWrapper>
                <Text
                  value="Waiting for output and `proof`" color=Colors.gray6 weight=Text.Regular
                />
              </div>
              <div className=Styles.resultWrapper>
                <ProgressBar
