module Styles = {
  ...
  let barItem = (width_, color_) =>
    style(list{width(#percent(width_)), height(#percent(100.)), backgroundColor(color_)})
}
...
let make = (~availableBalance, ~balanceAtStake, ~reward, ~unbonding, ~commission) => {
  ...
  <div className={Css.merge(list{Styles.barChart, CssHelper.flexBox()})}>
    ...
    {commission == 0.
      ? React.null
      : <div className={Styles.barItem(commissionPercent, Colors.gray6)} />}
  </div>
}
