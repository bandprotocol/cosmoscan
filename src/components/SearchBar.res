module Styles = {
  ...
  let container = style(list{
    ...
    Media.mobile(list{margin(#zero), display(#flex), maxWidth(#percent(100.))}),
  })
  let searchIcon = style(list{
    position(#absolute),
    top(#px(10)),
    left(#px(15)),
    width(#px(15)),
  ...
  let search = (theme: Theme.t) =>
    style(list{
      ...
      placeholder(list{color(theme.textSecondary)}),
      Media.mobile(list{fontSize(#px(10))}),
    })
...
module SearchResults = {
  module Styles = {
    ...
    let result = style(list{padding(Spacing.sm), paddingLeft(#px(38)), cursor(#pointer)})

    let lastResult = style(list{
    ...
    let resultFocused = style(list{backgroundColor(blueviolet), color(white)})
  }
