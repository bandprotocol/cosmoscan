module Styles = {
  ...
  let tableWrapper = style(list{Media.mobile(list{padding2(~v=#px(16), ~h=#zero)})})
  ...
  let toggle = style(list{cursor(#pointer)})
}
...
module DataSourceItem = {
  ...
  let make = (~dataSource: ReportSub.ValidatorReport.report_details_t) => {
    ...
    <Row>
      ...
      <Col col=Col.Two> <Text block=true value=dataSource.exitCode color=theme.textPrimary /> </Col>
      <Col col=Col.Three>
        <Text
          block=true
...
module RenderBodyMobile = {
  ...
  let make = (
    ~reserveIndex,
    ~reportsSub: ApolloHooks.Subscription.variant<ReportSub.ValidatorReport.t>,
  ) => {
    ...
    | Data({txHash, request: {id, oracleScript: {oracleScriptID, name}}, reportDetails}) =>
      <MobileCard
        ...
        panels={reportDetails
        ->Belt_Array.map(({externalID, exitCode, data, rawRequest: rawRequestOpt}) => {
          let {ReportSub.ValidatorReport.dataSource: {dataSourceID, dataSourceName}, calldata} =
...
let make = (~address) => {
  ...
  let reportsSub = ReportSub.ValidatorReport.getListByValidator(
    ~page,
    ~pageSize,
    ~validator=address |> Address.toOperatorBech32,
  )
