module Styles = {
  ...
  let card = (theme: Theme.t) =>
    style(list{
      ...
      Media.smallMobile(list{margin2(~v=#zero, ~h=#px(-5))}),
    })

  let innerCard = style(list{
    position(#relative),
    zIndex(2),
    minHeight(#px(177)),
    padding2(~v=#px(24), ~h=#px(32)),
    Media.mobile(list{padding2(~v=#px(10), ~h=#px(12)), minHeight(#px(146))}),
  })

  let fullWidth = style(list{width(#percent(100.))})

  let specialBg = style(list{
    backgroundImage(
      #linearGradient(
        #deg(270.),
...
module HighlightCard = {
  ...
  let make = (
    ~label,
    ~valueAndExtraComponentSub: ApolloHooks.Subscription.variant<_>,
    ~special=false,
  ) => {
    let ({ThemeContext.theme: theme}, _) = React.useContext(ThemeContext.context)
    let isMobile = Media.isMobile()

    <div className={Css.merge(list{Styles.card(theme), special ? Styles.specialBg : ""})}>
  ...
  <Row justify=Row.Between>
    <Col col=Col.Three colSm=Col.Six mbSm=16>
      <HighlightCard
        label="Band Price"
        special=true
        valueAndExtraComponentSub=%Sub({
          let (_, {financial}, _) = allSub
          (
            {
              let bandPriceInUSD = "$" ++ (financial.usdPrice |> Format.fPretty(~digits=2))
              <Text value=bandPriceInUSD size=Text.Xxxl weight=Text.Semibold color=theme.white />
            },
            {
    ...
    <Col col=Col.Three colSm=Col.Six>
      <HighlightCard
        label="Latest Block"
        valueAndExtraComponentSub=%Sub({
          let ({height, validator: {moniker, identity, operatorAddress}}, _, _) = allSub
          (
            <TypeID.Block id=height position=TypeID.Landing />,
            <ValidatorMonikerLink
              validatorAddress=operatorAddress moniker identity width=#percent(100.) avatarWidth=20
            />,
          ) |> Sub.resolve
        })
      />
    ...
    <Col col=Col.Three colSm=Col.Six>
      <HighlightCard
        ...
        valueAndExtraComponentSub=%Sub({
          let (_, _, (activeValidatorCount, bondedTokenCount)) = allSub
          (
            {
              let activeValidators = activeValidatorCount->Format.iPretty ++ " Nodes"
              <Text
                value=activeValidators size=Text.Xxxl color=theme.textPrimary weight=Text.Semibold
              />
            },
            <Text
              value={(bondedTokenCount
              |> Coin.getBandAmountFromCoin
              |> Format.fPretty) ++ " BAND Bonded"}
            />,
          ) |> Sub.resolve
        })
      />
