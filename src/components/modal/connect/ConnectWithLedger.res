module Styles = {
  ...
  let instructionCard = style(list{
    display(#flex),
    flexDirection(#row),
    alignItems(#center),
    height(#px(50)),
  ...
  let oval = style(list{
    backgroundImage(
      #linearGradient(
        #deg(180.),
        list{(#percent(0.), Colors.blue2), (#percent(100.), Colors.bandBlue)},
      ),
    ),
    width(#px(30)),
    height(#px(30)),
  ...
  let rFlex = style(list{display(#flex), flexDirection(#row), alignItems(#center)})

  let resultContainer = style(list{
    display(#flex),
    flexDirection(#row),
    alignItems(#center),
    justifyContent(#spaceBetween),
  ...
  let ledgerGuide = style(list{width(#px(248)), height(#px(38))})
  ...
  let connectingBtnContainer = style(list{
    width(#px(104)),
    display(#flex),
    justifyContent(#spaceBetween),
  })
  ...
  let createLedger = accountIndex => {
    ...
    let _ =
      ...
      |> Js.Promise.then_(wallet =>
        %Promise({
          let (address, pubKey) = wallet->Wallet.getAddressAndPubKey
      ...
      : <Button
          ...
          onClick={_ =>
            ...
            | (true, false) =>
              let isConfirm = {
                open Webapi.Dom
                window |> Window.confirm(j`To use Ledger Nano on Windows 10, please enable "Experimental Web Platform Features" by copy-paste "chrome://flags/#enable-experimental-web-platform-features". Click OK to copy.`)
              }
              isConfirm
