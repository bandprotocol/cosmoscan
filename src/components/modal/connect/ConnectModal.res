module Styles = {
  ...
  let innerContainer = style(list{display(#flex), flexDirection(#column), width(#percent(100.))})

  let loginSelectionContainer = style(list{padding2(~v=#zero, ~h=#px(24)), height(#percent(100.))})
  ...
  let row = style(list{height(#percent(100.))})
  let rowContainer = style(list{margin2(~v=#zero, ~h=#px(12)), height(#percent(100.))})
  ...
  let loginSelectionBackground = (theme: Theme.t) => style(list{background(theme.contrastBg)})

  let ledgerIcon = style(list{height(#px(28)), width(#px(28)), transform(translateY(#px(3)))})
  let ledgerImageContainer = active =>
    style(list{opacity(active ? 1.0 : 0.5), marginRight(#px(15))})
}
...
let toLoginMethodString = method_ =>
  switch method_ {
        ...
        <Row style=Styles.row>
          <Col col=Col.Five style={Styles.loginSelectionBackground(theme)}>
            <div className=Styles.loginSelectionContainer>
              ...
              ->Belt_Array.map(method_ =>
                <React.Fragment key={method_ |> toLoginMethodString}>
                  <VSpacing size=Spacing.lg />
                  <LoginMethod
                    name=method_
                    active={loginMethod == method_}
                    onClick={_ => setLoginMethod(_ => method_)}
                  />
                </React.Fragment>
              )
              ->React.array}
