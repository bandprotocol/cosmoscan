module Styles = {
  ...
  let connectContainer = (theme: Theme.t) =>
    style(list{height(#px(145)), backgroundColor(theme.secondaryBg)})
  ...
  let buttonContainer = style(list{
    selector("> button + button", list{marginLeft(#px(15))}),
    selector("> div + div", list{marginLeft(#px(15))}),
  })

  let linkStyle = style(list{color(#currentColor), transition(~duration=0, "all")})
}
      ...
      <Row>
        <Col col=Col.Six>
          <div className={CssHelper.mb(~size=16, ())}>
            <Heading
              value="Balance at Stake" size=Heading.H5 color=theme.textSecondary marginBottom=8
            />
            {switch allSub {
        ...
        <Col col=Col.Three>
          <div className={CssHelper.mb(~size=16, ())}>
            <div className={CssHelper.flexBox()}>
              <Heading
                value="Unbonding Amount" size=Heading.H5 marginBottom=8 color=theme.textSecondary
              />
            </div>
            {switch allSub {
