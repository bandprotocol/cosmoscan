module Styles = {
  ...
  let disable = isActive => style(list{display(isActive ? #flex : #none)})
  ...
  let selectContent = style(list{
    ...
    width(#px(135)),
    focus(list{outlineColor(Colors.white)}),
  })

  let nextBtn = style(list{width(#percent(100.)), marginTop(#px(24))})

  let info = style(list{display(#flex), justifyContent(#spaceBetween), alignItems(#center)})
}
      ...
      <div id="nextButtonContainer">
        <Button
          ...
          onClick={_ => {
            ...
            let _ = switch rawTxOpt {
            | Some(rawTxPromise) =>
              %Promise({
                let rawTx = rawTxPromise
            ...
            | None =>
              {
                open Webapi.Dom
                window |> Window.alert("invalid msgs")
              }
              Promise.ret()
            }
          }}>
