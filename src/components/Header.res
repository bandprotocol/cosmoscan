module Styles = {
  ...
  let bandLogo = style(list{
    width(#px(40)),
    marginRight(#px(5)),
    Media.mobile(list{width(#px(34))}),
  })

  let blockImage = style(list{display(#block)})

  let link = style(list{cursor(#pointer)})

  let chainIDContainer = style(list{marginLeft(#px(24))})
