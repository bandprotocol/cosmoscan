module Styles = {
  ...
  let container = style(list{position(#relative), paddingBottom(#px(24))})
  ...
  let errMsg = style(list{position(#absolute), bottom(#px(7))})
}
...
let make = (
  ...
  <div className=Styles.container>
            ...
            onClick={_ =>
              maxWarningMsg
                ? {
                    let isConfirm = {
                      open Webapi.Dom
                      window |> Window.confirm(j`You will not have balance to do any action after using max balance.`)
                    }
    ...
    {switch status {
    | Touched(Err(errMsg)) =>
      <div className=Styles.errMsg> <Text value=errMsg size=Text.Sm color=Theme.failColor /> </div>
    | _ => React.null
    }}
  </div>
}
