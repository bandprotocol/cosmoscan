...
module Styles = {
  ...
  let tabletContainer = (theme: Theme.t) =>
    style(list{
      ...
      borderRadius(#px(8)),
      Media.mobile(list{padding2(~v=#px(8), ~h=#px(12))}),
    })

  let tableSpacing = style(list{
    padding2(~v=#px(8), ~h=#zero),
    Media.mobile(list{padding2(~v=#px(4), ~h=#zero)}),
  })
...
let renderField = (field, maxWidth) =>
  ...
  | DataSource(id, name) =>
    <div className={Styles.valueContainer(maxWidth)}>
      ...
      <Text value=name weight=Text.Regular spacing=Text.Em(0.02) size=Text.Sm height=Text.Px(16) />
    </div>
  | Block(id) =>
    <div className={Styles.valueContainer(maxWidth)}>
...
@react.component
let make = (~headers=list{"Key", "Value"}, ~rows) => {
  ...
  <div className={Styles.tabletContainer(theme)}>
    ...
    ->Belt.List.mapWithIndex((i, row) =>
      <div
        key={"outerRow" ++ (i |> string_of_int)} className={Css.merge(list{Styles.tableSpacing})}>
        <Row>
          {row
