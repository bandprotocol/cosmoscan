  ...
  let make = (~connectionSub: ApolloHooks.Subscription.variant<ConnectionSub.t>) => {
    ...
    <div className={Styles.listContainer(theme)}>
      <Row alignItems=Row.Center>
        ...
        <Col col=Col.Two>
          {switch connectionSub {
          | Data({clientID}) => <div className={CssHelper.flexBox()}> <Text value=clientID /> </div>
          | _ => <LoadingCensorBar width=80 height=15 />
          }}
        </Col>
        <Col col=Col.Two>
...
module ConnectionListMobile = {
  module Styles = {
    ...
    let root = style(list{
      marginBottom(#px(8)),
      Media.mobile(list{padding4(~top=#px(22), ~left=#px(16), ~right=#px(16), ~bottom=#px(5))}),
    })
...
let make = (~counterpartyChainID) => {
  ...
  <>
    <Row alignItems=Row.Center marginTop=80 marginTopSm=36>
      <Col col=Col.Twelve>
        <div className={CssHelper.flexBox()}>
          <div className=Styles.greenDot /> <Heading value="Live Connection" size=Heading.H2 />
        </div>
      </Col>
    </Row>
