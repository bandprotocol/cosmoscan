module Styles = {
  ...
  let cardContainer = style(list{
    position(#relative),
    selector(" > div + div", list{marginTop(#px(12))}),
  })

  let infoContainer = style(list{height(#percent(100.))})

  let badgeContainer = style(list{selector(" > div", list{margin(#zero)})})

  let failText = style(list{cursor(#pointer), selector("> span", list{marginRight(#px(8))})})
}
  ...
  <InfoContainer py=24 style=Styles.infoContainer>
    <div className=Styles.cardContainer>
          ...
          | Some({status, reason}) =>
            ...
            | Fail =>
              <div
                ...
                <Text value="View Error Message" color=Theme.failColor /> <img src=Images.fail />
              </div>
            }
          | _ => React.null
          }
      ...
      | Data({
          ...
          | OracleRequest
          | FungibleToken =>
            <PacketDetail.TxHash txHash={txHash |> Belt.Option.getExn} />
          ...
          <PacketDetail.Common
            key={srcChannel ++
            (dstChannel ++
            (srcPort ++ (dstPort ++ (sequence |> string_of_int))))}
            srcChannel
            dstChannel
            srcPort
            dstPort
          ...
          | OracleRequest =>
            %Opt({
              let acknowledgement = acknowledgementOpt

              switch acknowledgement.data {
              | Request({requestID: requestIDOpt}) =>
                %Opt({
                  let requestID = requestIDOpt
