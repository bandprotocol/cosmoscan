module Styles = {
  ...
  let container = (show, theme: Theme.t) =>
    style(list{
      ...
      Media.mobile(list{padding2(~h=#px(16), ~v=#zero)}),
    })

  let thead = (theme: Theme.t) =>
    style(list{borderBottom(#px(1), #solid, theme.tableRowBorderColor)})
}
...
module RenderMobile = {
  module Styles = {
    ...
    let status = style(list{position(#absolute), top(#px(24)), right(#zero)})
      ...
      <div className={CssHelper.flexBox(~align=#center, ())}>
        <div className=Styles.labelWrapper>
          <div className={CssHelper.flexBox(~direction=#column, ~align=#flexStart, ())}>
            ...
            <Text value="Counterparty" size=Text.Sm transform=Text.Uppercase weight=Text.Semibold />
          </div>
        </div>
      ...
      : <Row alignItems=Row.Center minHeight=#px(40) style={Styles.thead(theme)}>
          <Col col=Col.Five>
            <div className={CssHelper.flexBox()}>
              ...
              <Icon name="fal fa-long-arrow-right" ml=8 mr=8 />
              <Text
                value="Counterparty Port" size=Text.Sm transform=Text.Uppercase weight=Text.Semibold
              />
            </div>
          </Col>
          <Col col=Col.Four>
