module Styles = {
  ...
  let valueWrapper = style(list{
    ...
    selector("i", list{margin2(~v=#zero, ~h=#px(8))}),
  })

  let status = style(list{width(#px(20)), height(#px(20))})
}
...
module ResolveStatus = {
  ...
  let make = (~status) =>
    <div className={CssHelper.flexBox(~align=#center, ())}>
      ...
      <div className=Styles.valueWrapper>
        {switch status {
        | IBCSub.OracleResponseData.Success => <img src=Images.success className=Styles.status />
        | Fail => <img src=Images.fail className=Styles.status />
        | _ => React.null
        }}
      </div>
    </div>
...
module Loading = {
  ...
  let make = () => <>
    <div className={CssHelper.flexBox(~align=#center, ())}>
      <div className=Styles.labelWrapper> <LoadingCensorBar width=100 height=16 radius=8 /> </div>
      <div className=Styles.valueWrapper>
        <div className={CssHelper.flexBox(~align=#center, ())}>
    ...
    <div className={CssHelper.flexBox(~align=#center, ())}>
      <div className=Styles.labelWrapper> <LoadingCensorBar width=100 height=16 radius=8 /> </div>
      <div className=Styles.valueWrapper> <LoadingCensorBar width=180 height=16 radius=8 /> </div>
    </div>
  </>
}
