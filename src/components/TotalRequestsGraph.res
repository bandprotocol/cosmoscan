module Styles = {
  ...
  let card = (theme: Theme.t) =>
    style(list{
      ...
      Media.mobile(list{padding2(~v=#px(24), ~h=#px(16))}),
    })

  let innerCard = style(list{width(#percent(100.)), height(#px(220)), margin2(~v=#zero, ~h=#auto)})

  let requestCount = style(list{marginTop(#px(8))})

  let chart = show => style(list{important(display(show ? #block : #none))})
}

let renderGraph: (array<HistoricalTotalRequestQuery.t>, bool) => unit = %raw(`
...
let make = (~latest5RequestSub: Sub.t<array<RequestSub.t>>) => {
  ...
  <div className={Styles.card(theme)}>
    <div
      ...
      className={Css.merge(list{CssHelper.mb(~size=40, ()), CssHelper.mbSm(~size=16, ())})}>
