...
let get = () => {
  ...
  %Query({
    ...
    let x = result["request_count_per_days"]->Belt.Array.map(each => {
      t: each["date"] |> GraphQLParser.timestamp |> MomentRe.Moment.toUnix,
      y: each["count"],
    })

    let (cumulativeSum, _) = x->Belt.Array.reduce(([], 0), ((ls, sum), each) => {
