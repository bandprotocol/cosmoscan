...
let count = (proposalID, answer) => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=VoteCountConfig.makeVariables(
      ~proposal_id=proposalID |> ID.Proposal.toInt,
      ~answer=answer |> toString |> Js.Json.string,
      (),
    ),
...
let parse = (valVotes, delVotes) => {
  let valVotesMap = valVotes->Belt_Array.reduce(Belt_MapString.empty, (
    acc,
    {validatorID, answer, power, proposalID},
  ) =>
  ...
  let valVotesWithDelVotesMap =
    ...
    ->Belt_Array.reduce(valVotesMap, (acc, {validatorID, answer, power, proposalID}) =>
      acc->Belt_MapString.update(
        (validatorID |> string_of_int) ++ (proposalID |> ID.Proposal.toString),
        v => {
          let entry = v->Belt_Option.getWithDefault({
            validatorID: validatorID,
            valPower: 0.,
            valVote: None,
            delVotes: _ => 0.,
  ...
  let parsedData =
    ...
    ->Belt_Array.reduce(Belt_MapString.empty, (
      acc,
      {validatorID, valPower, valVote, delVotes, proposalID},
    ) =>
      acc->Belt_MapString.set(
        (validatorID |> string_of_int) ++ (proposalID |> ID.Proposal.toString),
        {
          validatorID: validatorID,
          validatorPower: switch valVote {
          | Some(_) =>
            valPower -. delVotes(Yes) -. delVotes(No) -. delVotes(NoWithVeto) -. delVotes(Abstain)
          | None => 0.
          },
...
let getVoteStatByProposalID = proposalID => {
  let (validatorVotes, _) = ApolloHooks.useSubscription(
    ValidatorVoteByProposalIDConfig.definition,
    ~variables=ValidatorVoteByProposalIDConfig.makeVariables(
      ~proposal_id=proposalID |> ID.Proposal.toInt,
      (),
    ),
  )
  let (delegatorVotes, _) = ApolloHooks.useSubscription(
    DelegatorVoteByProposalIDConfig.definition,
    ~variables=DelegatorVoteByProposalIDConfig.makeVariables(
      ~proposal_id=proposalID |> ID.Proposal.toInt,
      (),
    ),
  )
  ...
  %Sub({
    let valVotes = validatorVotes
    %Sub({
      let delVotes = delegatorVotes
      ...
      let delegatorPower =
        delVotes["non_validator_vote_proposals_view"]->Belt_Array.reduce(0., (a, {power}) =>
          a +. (power |> Coin.getBandAmountFromCoin)
        )
        ...
        ->Belt_Array.reduce(Belt_MapInt.empty, (acc, {validatorID, proposalID}) =>
          acc->Belt_MapInt.set(
            proposalID |> ID.Proposal.toInt,
            {
              proposalID: proposalID,
              validatorID: validatorID,
              validatorPower: parsedData->Belt_Array.reduce(0., (
                a,
                {validatorPower, proposalID: delProposalID},
      ...
      let voteMap =
        parsedMap->Belt_Array.reduce(Belt_MapInt.empty, (acc, {proposalID, validatorPower}) =>
          acc->Belt_MapInt.set(
            proposalID |> ID.Proposal.toInt,
            validatorPower +.
            delVotes["non_validator_vote_proposals_view"]->Belt_Array.reduce(0., (
              a,
              {power, proposalID: delProposalID},
