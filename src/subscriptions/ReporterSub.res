...
let getList = (~operatorAddress, ~page, ~pageSize, ()) => {
  ...
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=ReportersByValidatorAddressConfig.makeVariables(
...
let count = operatorAddress => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=ReportersCountConfig.makeVariables(
      ~operator_address=operatorAddress |> Address.toOperatorBech32,
      (),
    ),
  )
  result |> Sub.map(_, x =>
    x["reporters_aggregate"]["aggregate"] |> Belt_Option.getExn |> (y => y["count"])
  )
}
