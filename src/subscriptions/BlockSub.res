...
let getListByConsensusAddress = (~address, ~page, ~pageSize, ()) => {
  ...
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=MultiConsensusAddressConfig.makeVariables(
...
let getLatest = () =>
  %Sub({
    let blocks = getList(~pageSize=1, ~page=1, ())
...
let count = () => {
  ...
  result |> Sub.map(_, x =>
    x["blocks_aggregate"]["aggregate"] |> Belt_Option.getExn |> (y => y["count"])
  )
}
...
let getAvgBlockTime = (greater, less) => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=PastDayBlockCountConfig.makeVariables(
...
let countByConsensusAddress = (~address, ()) => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=BlockCountConsensusAddressConfig.makeVariables(
      ~address=address |> Address.toHex,
      (),
    ),
  )
