...
let getStakeList = (delegatorAddress, ~page, ~pageSize, ()) => {
  ...
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=StakeConfig.makeVariables(
...
let getTotalStakeByDelegator = delegatorAddress => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=TotalStakeByDelegatorConfig.makeVariables(
      ~delegator_address=delegatorAddress |> Address.toBech32,
      (),
    ),
  )
  ...
  let delegatorInfoSub =
    result |> Sub.map(_, a =>
      (
        a["delegations_view_aggregate"]["aggregate"] |> Belt_Option.getExn
      )["sum"] |> Belt_Option.getExn
    )

  %Sub({
    let delegatorInfo = delegatorInfoSub
...
let getStakeByValidator = (delegatorAddress, operatorAddress) => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=StakeByValidatorConfig.makeVariables(
...
let getStakeCountByDelegator = delegatorAddress => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=StakeCountByDelegatorConfig.makeVariables(
      ~delegator_address=delegatorAddress |> Address.toBech32,
      (),
    ),
  )
...
let getDelegatorsByValidator = (validatorAddress, ~page, ~pageSize, ()) => {
  ...
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=DelegatorsByValidatorConfig.makeVariables(
...
let getDelegatorCountByValidator = validatorAddress => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=DelegatorCountConfig.makeVariables(
      ~operator_address=validatorAddress |> Address.toOperatorBech32,
      (),
    ),
  )
