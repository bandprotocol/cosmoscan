...
let get = operator_address => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=SingleConfig.makeVariables(
...
let count = () => {
  ...
  result |> Sub.map(_, x =>
    x["validators_aggregate"]["aggregate"] |> Belt_Option.getExn |> (y => y["count"])
  )
}
...
let getTotalBondedAmount = () => {
  ...
  result |> Sub.map(_, a =>
    (
      (a["validators_aggregate"]["aggregate"] |> Belt_Option.getExn)["sum"] |> Belt_Option.getExn
    )["tokens"]
  )
}
...
let getUptime = consensusAddress => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=SingleLast100VotedConfig.makeVariables(
  ...
  %Sub({
    ...
    let signedBlock =
      ...
      ->Belt.Option.mapWithDefault(0, GraphQLParser.int64) |> float_of_int
    let missedBlock =
      validatorVotes
...
let getListVotesBlock = () => {
  ...
  %Sub({
    let x = result
    let validatorVotes = x["validator_last_100_votes"]->Belt.Array.map(each => {
...
let getBlockUptimeByValidator = consensusAddress => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=SingleLast100ListConfig.makeVariables(
      ~consensusAddress=consensusAddress |> Address.toHex,
      (),
    ),
  )
  %Sub({
    let x = result
    let validatorVotes = x["validator_votes"]->Belt.Array.map(each => {
    ...
    Sub.resolve({
      ...
      proposedCount: validatorVotes
      ->Belt.Array.keep(({status}) => status == Proposed)
      ->Belt.Array.size,
      signedCount: validatorVotes->Belt.Array.keep(({status}) => status == Signed)->Belt.Array.size,
      missedCount: validatorVotes->Belt.Array.keep(({status}) => status == Missed)->Belt.Array.size,
    })
  })
}
...
let getHistoricalOracleStatus = (operatorAddress, greater, oracleStatus) => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=HistoricalOracleStatusesConfig.makeVariables(
    ...
    let oracleStatusReports =
      x["historical_oracle_statuses"]->Belt.Array.size > 0
        ? x["historical_oracle_statuses"]
          ...
          ->Belt.List.fromArray
        : list{{timestamp: startDate, status: oracleStatus}}

    let rawParsedReports = HistoryOracleParser.parse(~oracleStatusReports, ~startDate, ())

    let parsedReports = if !oracleStatus && x["historical_oracle_statuses"]->Belt.Array.size == 0 {
      rawParsedReports->Belt.Array.map(({timestamp}) => {
        HistoryOracleParser.timestamp: timestamp,
        status: false,
      })
    } else {
      rawParsedReports
