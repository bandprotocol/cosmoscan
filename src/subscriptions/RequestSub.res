...
module Mini = {
  ...
  let getListByDataSource = (id, ~page, ~pageSize, ()) => {
    ...
    let (result, _) = ApolloHooks.useSubscription(
      ...
      ~variables=MultiMiniByDataSourceConfig.makeVariables(
  ...
  let getListByOracleScript = (id, ~page, ~pageSize, ()) => {
    ...
    let (result, _) = ApolloHooks.useSubscription(
      ...
      ~variables=MultiMiniByOracleScriptConfig.makeVariables(
  ...
  let getListByTxHash = (txHash: Hash.t) => {
    let (result, _) = ApolloHooks.useSubscription(
      ...
      ~variables=MultiMiniByTxHashConfig.makeVariables(
...
let count = () => {
  ...
  result |> Sub.map(_, x =>
    x["requests_aggregate"]["aggregate"] |> Belt_Option.getExn |> (y => y["count"])
  )
}
...
let countByOracleScript = id => {
  let (result, _) = ApolloHooks.useSubscription(
    ...
    ~variables=RequestCountByOracleScriptConfig.makeVariables(~id=id |> ID.OracleScript.toInt, ()),
  )
