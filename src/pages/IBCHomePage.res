module Styles = {
  ...
  let selectContainer = style(list{maxWidth(px(285))})
  let buttonContainer = style(list{height(#percent(100.))})
}
...
module CounterPartySelect = {
  ...
  let make = (~filterChainIDList: array<IBCFilterSub.filter_counterparty_t>, ~setChainID) => {
    ...
    let validatorList = filterChainIDList->Belt_Array.map(({chainID}) => {
      open ReactSelect
      {value: chainID, label: chainID}
    })
      ...
      <ReactSelect
        ...
        styles={
          ReactSelect.control: _ => {
            ...
            borderRadius: "8px",
            border: "1px solid" ++ (isDarkMode ? "#353535" : "#EDEDED"),
          },
