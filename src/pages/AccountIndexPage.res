module Styles = {
  ...
  let squareIcon = color =>
    style(list{width(#px(8)), marginRight(#px(8)), height(#px(8)), backgroundColor(color)})

  let balance = style(list{minWidth(#px(150)), justifyContent(#flexEnd)})

  let infoHeader = style(list{borderBottom(#px(1), #solid, Colors.gray9), paddingBottom(#px(16))})

  let totalBalance = style(list{
    display(#flex),
  ...
  let infoLeft = style(list{
    ...
    selector(
      ...
      list{
        ...
        width(#percent(100.)),
        Media.mobile(list{height(#auto), marginBottom(#px(16))}),
      },
    ),
  })

  let amountBoxes = style(list{
    selector("> div:nth-child(1)", list{marginBottom(#px(40))}),
    selector("> div + div", list{marginTop(#px(24))}),
  })

  let detailContainer = style(list{height(#percent(100.))})

  let addressContainer = style(list{width(#percent(100.)), maxWidth(#px(420))})

  let buttonContainer = style(list{
    marginTop(#px(24)),
    selector("> button + *", list{marginLeft(#px(16))}),
  })
}
    ...
    <Row>
      ...
      <Col col=Col.Six colSm=Col.Seven>
        <div className={CssHelper.flexBox(~direction=#column, ~align=#flexEnd, ())}>
          ...
          <div className={Css.merge(list{CssHelper.flexBox(), Styles.balance})}>
            {isCountup
              ? <NumberCountup
