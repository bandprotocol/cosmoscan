module Styles = {
  open Css
  let titleSpacing = style(list{marginBottom(#px(26))})
  let idCointainer = style(list{marginBottom(#px(16))})
  let containerSpacingSm = style(list{Media.mobile(list{marginTop(#px(16))})})

  let relatedDSContainer = style(list{
    selector("> div + div", list{marginTop(#px(16))}),
    selector("> div > a", list{marginRight(#px(8))}),
  })
}
                        ...
                        ? relatedDataSources
                          ->Belt.List.map(({dataSourceName, dataSourceID}) =>
                            <div
                              ...
                              <Text
                                value=dataSourceName size=Text.Lg block=true color=theme.textPrimary
                              />
                            </div>
                          )
                          ->Belt.List.toArray
