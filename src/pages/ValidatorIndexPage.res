module Styles = {
  open Css
  let card = style(list{height(#percent(100.)), padding2(~v=#px(24), ~h=#px(32))})

  let link = style(list{fontSize(#px(14))})

  let bondedTokenContainer = style(list{height(#percent(100.))})
  let avatarContainer = style(list{
    position(#relative),
    marginRight(#px(24)),
    Media.mobile(list{marginRight(#zero), marginBottom(#px(16))}),
  })
  let rankContainer = style(list{
  ...
  let customContainer = style(list{height(#percent(100.))})

  let chartWrapper = style(list{minHeight(px(220)), selector("> div", list{width(#percent(100.))})})
}
    ...
    <div className=CssHelper.container>
            ...
            | Data(({isActive}, _, _)) =>
              <div className={CssHelper.flexBox()}>
                <div className={CssHelper.flexBox(~justify=#center, ())}>
                  <img src={isActive ? Images.activeValidatorLogo : Images.inactiveValidatorLogo} />
                </div>
        ...
        <Col col=Col.Three colSm=Col.Six mbSm=16>
          <InfoContainer style=Styles.card>
            <div className={Css.merge(list{CssHelper.flexBox(), CssHelper.mb(~size=27, ())})}>
              <Heading
                value="Oracle Reports" size=Heading.H4 color=theme.textSecondary weight=Heading.Thin
              />
        ...
        <Col>
          <InfoContainer>
            ...
            <Row>
              <Col col=Col.Four mbSm=8>
                <Heading
                  value="Description" size=Heading.H4 weight=Heading.Thin color=theme.textSecondary
                />
              </Col>
              <Col col=Col.Eight>
                {switch allSub {
      ...
      <Table>
        <Tab.Route
          ...
          | ProposedBlocks =>
            switch validatorSub {
            | Data(validator) => <ProposedBlocksTable consensusAddress=validator.consensusAddress />
