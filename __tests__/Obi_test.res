...
describe("Expect Obi to extract fields correctly", () => {
  test("should be able to extract fields from bytes correctly", () =>
    ...
    ) |> toEqual(extractFields(j`{symbol:string,multiplier:u64}/{volume:u64}`, "input"))
  )

  test("should return None on invalid type", () =>
    expect(None) |> toEqual(extractFields(j`{symbol:string,multiplier:u64}/{volume:u64}`, "Input"))
  )
})
...
describe("Expect Obi encode correctly", () => {
  test("should be able to encode input (string, int) correctly", () =>
    expect(Some("0x00000003425443000000003b9aca00" |> JsBuffer.fromHex)) |> toEqual(
      encode(
        j`{symbol: string,multiplier: u64}/{price: u64,sources: [{ name: string, time: u64 }]}`,
        "input",
        [
  ...
  test("should be able to encode nested input correctly", () =>
    expect(Some("0x00000001000000020000000358585800000003595959" |> JsBuffer.fromHex)) |> toEqual(
      encode(
        j`{list: [{symbol: {name: [string]}}]}/{price: u64}`,
        "input",
        [{fieldName: "list", fieldValue: j`[{"symbol": {"name": ["XXX", "YYY"]}}]`}],
      ),
    )
  )
  ...
  test("should be able to encode output correctly", () =>
    expect(Some("0x0000000200000000000000780000000000000143" |> JsBuffer.fromHex)) |> toEqual(
      encode(
        j`{list: [{symbol: {name: [string]}}]}/{price: [u64]}`,
        "output",
        [{fieldName: "price", fieldValue: j`[120, 323]`}],
      ),
    )
  )
  ...
  test("should return None if invalid input schema", () =>
    expect(None) |> toEqual(
      encode(
        j`{symbol: string}/{price: u64,sources: [{ name: string, time: u64 }]}`,
        "nothing",
        [
          {fieldName: "symbol", fieldValue: "BTC"},
          {fieldName: "multiplier", fieldValue: "1000000000"},
  ...
  test("should return None if invalid output schema", () =>
    expect(None) |> toEqual(
      encode(j`{symbol: string}`, "nothing", [{fieldName: "symbol", fieldValue: "BTC"}]),
    )
  )
})
...
describe("Expect Obi decode correctly", () => {
  ...
  test("should be able to decode from bytes correctly (bytes)", () =>
    expect(Some([{fieldName: "symbol", fieldValue: "0x55555555"}])) |> toEqual(
      decode(j`{symbol: bytes}/{price: u64}`, "input", "0x0000000455555555" |> JsBuffer.fromHex),
    )
  )
...
`),
    ) |> toEqual(generateDecoderSolidity(j`{symbol:string,multiplier:u64}/{px:u64}`))
  )
...
`),
    ) |> toEqual(generateDecoderSolidity(j`{symbols:[string],multiplier:u64}/{rates:[u64]}`))
  )
})
...
describe("should be able to generate go code correctly", () => {
  ...
  test("should be able to generate go code 1", () =>
    expect(Some(j`"Code is not available."`)) |> toEqual(
      generateDecoderGo("main", j`{symbol:string,multiplier:u64}/{px:u64}`, Obi.Params),
    )
  )
...
func DecodeResult(data []byte) (Result, error) {
    ...
    ) |> toEqual(generateDecoderGo("test", j`{symbol:string,multiplier:u64}/{px:u64}`, Obi.Result))
  )
})
...
func(result *Result) EncodeResult() []byte {
    ...
    ) |> toEqual(generateEncodeGo("main", j`{symbol:string,multiplier:u64}/{px:u64}`, "input"))
  )
...
func(result *Result) EncodeResult() []byte {
    ...
    ) |> toEqual(generateEncodeGo("test", j`{symbol:string,multiplier:u64}/{px:u64}`, "output"))
  )
})
